...
#caption{position:absolute;right:4%;bottom:6%;left:4%;display:none;padding:14px 22px;border-radius:4px;color:#fff;text-align:center;background:var(--bg);line-height:var(--spacing);font-family:"Microsoft YaHei","Segoe UI",sans-serif}
#caption.top{top:6%;bottom:auto}#caption.center{top:50%;bottom:auto;transform:translateY(-50%)}#caption.side{display:flex;align-items:baseline;justify-content:center;gap:22px}
...
<script>
const box=document.getElementById("caption"),cn=document.getElementById("cn"),en=document.getElementById("en");let style={},last=null;
...
function apply(s){style=s||{};document.documentElement.style.setProperty("--cn-size",(style.chinese?.fontSize||34)+"px");document.documentElement.style.setProperty("--en-size",(style.english?.fontSize||23)+"px");document.documentElement.style.setProperty("--cn-color",style.chinese?.color||"#fff");document.documentElement.style.setProperty("--en-color",style.english?.color||"#a7f3d0");document.documentElement.style.setProperty("--cn-opacity",style.chinese?.opacity??1);document.documentElement.style.setProperty("--en-opacity",style.english?.opacity??.94);document.documentElement.style.setProperty("--bg",rgba(style.backgroundColor,style.backgroundOpacity??.68));document.documentElement.style.setProperty("--outline",(style.outlineWidth||2)+"px");document.documentElement.style.setProperty("--outline-color",style.outlineColor||"#020617");document.documentElement.style.setProperty("--spacing",style.lineSpacing||1.3);box.style.fontFamily=style.chinese?.fontFamily||"Microsoft YaHei,sans-serif";box.style.maxWidth=(style.maxWidth||1100)+"px";box.classList.toggle("top",style.position==="top");box.classList.toggle("center",style.position==="center");box.classList.toggle("side",style.layout==="sideBySide");box.style.textAlign=style.alignment||"center";render()}
function render(){if(!last){box.style.display="none";return}if(last.kind==="partial"&&!style.showTemporary||last.kind==="final"&&!style.showFinal){box.style.display="none";return}cn.textContent=last.chinese||"";en.textContent=last.english||"";en.style.display=last.english?"":"none";box.classList.toggle("temporary",last.kind==="partial");box.style.display=last.chinese?(style.layout==="sideBySide"?"flex":"block"):"none"}
...
Object.entries(fields).forEach(([id,path])=>$(id).addEventListener("input",e=>{let v=e.target.value;if(id==="cnSize"||id==="enSize")v=Number(v);if(id==="bgOpacity")v=Number(v)/100;set(style,path,v);render();queueSave()}));
