...
function render(){if(!last){box.style.display="none";return}if(last.kind==="partial"&&!style.showTemporary||last.kind==="final"&&!style.showFinal){box.style.display="none";return}swapText(cn,last.chinese||"");swapText(en,last.english||"");en.style.display=last.english?"":"none";box.classList.toggle("temporary",last.kind==="partial");box.style.display=last.chinese?(style.layout==="sideBySide"?"flex":"block"):"none"}
