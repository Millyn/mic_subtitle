...
const cloneConfig = (config: AppConfig): AppConfig => ({
  ...
  recognitionLanguage: config.recognitionLanguage || "auto",
...
const isEnglishLanguage = (language?: string) => {
  ...
  return normalized === "english" || normalized === "en" || normalized.startsWith("en-") || normalized.startsWith("english ");
};
  ...
  useEffect(() => {
    if (!desktopRuntime()) {
      if (recognitionStatus === "running") {
        const timer = window.setInterval(() => setAudioLevel(0.22 + Math.random() * 0.5), 200);
    ...
    const register = async () => {
      ...
      const subscriptions: Array<[string, () => Promise<UnlistenLike>]> = [
        ...
        ["subtitle", () => onSubtitle((event) => {
          ...
          if (event.kind === "final") {
            const englishSource = isEnglishLanguage(event.language);
            addLog(event.english ? `${englishSource ? "英文" : "中文"}句子已翻译并同步` : `已确认一条${englishSource ? "英文" : "中文"}句子`, event.english ? "success" : "neutral");
          }
        })],
    ...
    <div className="app-shell">
      <aside className="sidebar">
        ...
        <div className="sidebar-bottom">
          ...
          <div className="version">声译 v0.1.22 <span>·</span> Windows 版</div>
        </div>
      </aside>
...
function DashboardPage(props: {
  ...
  const englishSource = isEnglishLanguage(currentSubtitle?.language);
  const hasTranslation = Boolean(currentSubtitle?.english);
      ...
      <section className="panel caption-panel">
        ...
        <div className="caption-stage" style={{ textAlign: config.style.alignment }}>
          ...
          <div className={`caption-display ${config.style.position}`}>
            {currentSubtitle?.chinese ? <div className={`${englishSource ? "caption-en" : "caption-cn"} ${currentSubtitle.kind === "partial" ? "temporary" : ""}`}>{currentSubtitle.chinese}</div> : <div className="caption-empty"><div className="wave-placeholder"><i /><i /><i /><i /><i /></div><span>开始说话后，字幕会出现在这里</span></div>}
            {hasTranslation && <div className={englishSource ? "caption-cn" : "caption-en"}>{currentSubtitle?.english}</div>}
          </div>
          <div className="stage-badge"><span className="live-dot" />{currentSubtitle?.kind === "partial" ? "临时字幕" : "实时预览"}</div>
        </div>
        <div className="caption-footer"><div className="caption-stat"><span className="tiny-avatar">{englishSource ? "英" : "中"}</span><div><b>{currentSubtitle?.chinese ? `${englishSource ? "英文" : "中文"}识别` : "等待输入"}</b><small>{currentSubtitle?.chinese ? (currentSubtitle.kind === "partial" ? "正在聆听…" : "已确认") : "本地 ASR 引擎"}</small></div></div><div className="caption-actions"><button onClick={props.onTestTranslation}>测试翻译</button><button onClick={() => props.onOpenPage("style")}>编辑样式 <Icon name="chevron" size={13} /></button></div></div>
      </section>
    </div>
...
function GlossaryPage({ entries, isSaving, onChange, onSave }: { entries: GlossaryEntry[]; isSaving: boolean; onChange: (entries: GlossaryEntry[]) => void; onSave: (entries: GlossaryEntry[]) => void }) {
  ...
  const exportJson = async () => {
    const validEntries = validGlossaryEntries(entries);
    try {
      if (desktopRuntime()) {
        const path = await exportGlossary(validEntries);
        setMessage(`术语表已保存到程序目录：${path}`);
        return;
      }
      const payload = JSON.stringify({ version: 1, entries: validEntries }, null, 2);
      ...
      setMessage(errorMessage(error, "术语表导出失败"));
    }
  };
  ...
  return <div className="subpage glossary-page"><div className="subpage-intro"><div><p className="section-kicker">LOCAL TERMS</p><h2>术语表管理</h2><p>在本地维护固定译法。完整术语表不会发送给 AI，只有命中的术语会参与当前句子处理。</p></div><div className="glossary-actions"><button className="outline-button" onClick={() => fileInput.current?.click()}>导入 JSON</button><input ref={fileInput} className="hidden-file-input" type="file" accept="application/json,.json" onChange={(event) => void importJson(event)} /><button className="outline-button" onClick={() => void exportJson()}>导出 JSON</button><button className="dark-button" onClick={() => onSave(entries)} disabled={isSaving}><Icon name="check" size={14} />{isSaving ? "保存中…" : "保存术语表"}</button></div></div><section className="panel glossary-panel"><div className="glossary-toolbar"><div><b>{validGlossaryEntries(entries).length}</b><span>条有效术语</span></div><label className="glossary-search"><Icon name="grid" size={14} /><input value={query} placeholder="搜索中文或英文术语" onChange={(event) => setQuery(event.target.value)} /></label><button className="dark-button" onClick={addEntry}>+ 新增术语</button></div><div className="glossary-hint"><Icon name="shield" size={14} /><span>格式示例：<code>英伟达广播=NVIDIA Broadcast</code>。术语数量不设上限；单独识别到完整术语时可本地直出，不消耗 API Token。</span></div><div className="glossary-table"><div className="glossary-row glossary-head"><span>中文 / 原文</span><span>固定译法</span><span>操作</span></div>{visibleEntries.length ? visibleEntries.map(({ entry, index }) => <div className="glossary-row" key={`glossary-${index}`}><input value={entry.source} placeholder="例如：英伟达广播" onChange={(event) => updateEntry(index, { source: event.target.value })} /><input value={entry.target} placeholder="例如：NVIDIA Broadcast" onChange={(event) => updateEntry(index, { target: event.target.value })} /><button className="ghost-danger" onClick={() => removeEntry(index)} title="删除术语"><Icon name="trash" size={15} /></button></div>) : <div className="glossary-empty">没有匹配的术语，点击“新增术语”开始维护。</div>}</div><div className="glossary-footer"><span>{message}</span><small>不限制术语条数；保存和导出时会自动去除空行。</small></div></section></div>;
}
...
function LegacySettingsConnectionPage({ config, server, isSaving, onChange, onSave, onTest }: { config: AppConfig; server: ServerStatus; isSaving: boolean; onChange: (patch: Partial<AppConfig>) => void; onSave: () => void; onTest: () => void }) {
...
function SettingsConnectionPage({ config, server, isSaving, onChange, onSave, onTest }: { config: AppConfig; server: ServerStatus; isSaving: boolean; onChange: (patch: Partial<AppConfig>) => void; onSave: () => void; onTest: () => void }) {
  ...
  return <div className="subpage settings-page"><div className="subpage-intro"><div><p className="section-kicker">PRIVATE CONNECTIONS</p><h2>控制你的字幕管线</h2><p>API Key 只保存在本机配置中，不会发送到 OBS 字幕页面。</p></div><button className="dark-button save-settings" onClick={onSave} disabled={isSaving}>{isSaving ? "保存中…" : "保存全部设置"}</button></div><div className="settings-grid"><section className="panel settings-card"><PanelHeading eyebrow="TRANSLATION" title="DeepSeek 翻译" action={<button className={`toggle ${config.deepseek.enabled ? "on" : ""}`} onClick={() => onChange({ deepseek: { ...config.deepseek, enabled: !config.deepseek.enabled } })}><i /></button>} /><p className="card-description">确认识别到的中文或英文句子后发送到 DeepSeek，翻译失败时仍保留原文字幕。</p><label className="form-label">识别语言<select value={config.recognitionLanguage || "auto"} onChange={(e) => onChange({ recognitionLanguage: e.target.value as AppConfig["recognitionLanguage"] })}><option value="auto">自动识别中文/英文（推荐）</option><option value="Chinese">中文优先</option><option value="Chinese,English">中英混合（自动检测）</option></select></label><label className="form-label">API Key<input type="password" value={config.deepseek.apiKey} placeholder="sk-…" onChange={(e) => onChange({ deepseek: { ...config.deepseek, apiKey: e.target.value } })} autoComplete="off" /></label><label className="form-label">API 地址<input value={config.deepseek.baseUrl} onChange={(e) => onChange({ deepseek: { ...config.deepseek, baseUrl: e.target.value } })} /></label><label className="form-label">模型名<input value={config.deepseek.model} onChange={(e) => onChange({ deepseek: { ...config.deepseek, model: e.target.value } })} /></label><div className="settings-note"><Icon name="book" size={15} />术语表已移至左侧独立的“术语表”页面，可在那里进行增删改查、JSON 导入和导出。</div><button className="outline-button full-button" onClick={onTest}><Icon name="link" size={14} />发送翻译测试</button><div className="security-callout"><Icon name="shield" size={16} /><span><b>密钥隔离</b><small>翻译请求由桌面端发出，OBS 只能看到翻译结果。</small></span></div></section><section className="panel settings-card"><PanelHeading eyebrow="SERVICE" title="局域网字幕服务" /><p className="card-description">服务监听所有网卡，OBS 可以从本机或同一局域网的其他设备访问。</p><label className="form-label">监听端口<input type="number" min={1024} max={65535} value={config.serverPort} onChange={(e) => onChange({ serverPort: Number(e.target.value) || 39071 })} /></label><div className="service-preview"><div><span>OBS 字幕页面</span><b>{overlayUrl}</b></div><span className={`service-live ${server.running ? "" : "offline"}`}><i />{server.running ? "监听中" : "启动中"}</span></div><div className="service-preview"><div><span>样式编辑器</span><b>{editorUrl}</b></div><span className="service-live"><i />可访问</span></div><div className="settings-note"><Icon name="check" size={15} />监听地址：0.0.0.0:{server.port || config.serverPort} · Windows 防火墙首次提示请选择“专用网络”</div></section><section className="panel settings-card canvas-card"><PanelHeading eyebrow="OBS CANVAS" title="画布与字幕位置" /><p className="card-description">Overlay 以画布左上角为原点，X/Y 表示字幕框中心点。OBS 浏览器源建议设置为相同的画布比例。</p><div className="two-fields canvas-fields"><label>画布宽度<input type="number" min={320} max={16384} value={style.canvasWidth} onChange={(e) => updateCanvas("canvasWidth", Number(e.target.value))} /></label><label>画布高度<input type="number" min={180} max={8640} value={style.canvasHeight} onChange={(e) => updateCanvas("canvasHeight", Number(e.target.value))} /></label></div><div className="two-fields canvas-fields"><label>字幕 X<input type="number" min={0} max={style.canvasWidth} value={style.subtitleX} onChange={(e) => updateAnchor("subtitleX", Number(e.target.value))} /></label><label>字幕 Y<input type="number" min={0} max={style.canvasHeight} value={style.subtitleY} onChange={(e) => updateAnchor("subtitleY", Number(e.target.value))} /></label></div><div className="position-presets"><span>快速定位</span><button className={style.position === "top" ? "active" : ""} onClick={() => applyPreset("top")}>顶部</button><button className={style.position === "center" ? "active" : ""} onClick={() => applyPreset("center")}>居中</button><button className={style.position === "bottom" ? "active" : ""} onClick={() => applyPreset("bottom")}>底部</button></div><label className="form-label">无字幕自动隐藏（秒）<input type="number" min={1} max={300} value={style.hideAfterSeconds} onChange={(e) => updateStyle({ hideAfterSeconds: Math.max(1, Math.min(300, Math.round(Number(e.target.value) || 10))) })} /><small className="form-hint">Overlay 收到最后一条字幕后，超过该时间没有新字幕就自动隐藏；不会停止识别。</small></label><div className="settings-note"><Icon name="check" size={15} />当前锚点：{style.subtitleX} × {style.subtitleY} · 保存后实时同步到 OBS</div></section></div></div>;
}
